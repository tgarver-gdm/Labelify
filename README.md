...
- **The brief shipped no sample data**, so I pulled real labels from the public
  ...
  artwork), so this is photo-based OCR, not font-metadata reading.
...
- **ABV and net contents aren't in the registry's structured data** — they're only
...
- Prototype scope: no login, no database, no audit trail.
